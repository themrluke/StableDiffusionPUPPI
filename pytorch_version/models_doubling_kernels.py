...
class UNetLite_hls(nn.Module):
    ...
    def __init__(self, c_in=1, c_out=1, time_dim=4, device="cuda"): # Changed from cpu to cuda
        ...
        N = int(8) # Can alter number of kernels in each convolutional layer here
        ...
        self.convd2_1 = nn.Conv2d(N, N*2, kernel_size=3, padding=0, bias=False)
        self.normd2_2 = nn.GroupNorm(N, N*2, eps=1e-05, affine=True) # Make sure this 6 is the same as the  in the prev line. ie 6*N if the 6 above gets changed to 6*N
        #relu
        self.convd2_2 = nn.Conv2d(N*2, N*2, kernel_size=3, padding=0, bias=False)
        self.normd2_3 = nn.GroupNorm(N, N*2, eps=1e-05, affine=True)
        #relu
        self.pool2 = nn.Conv2d(N*2 , N*2, kernel_size=3, stride=2, padding=0, bias=False)

        '''Bottleneck'''
        self.normb1_1 = nn.GroupNorm(N, N*2, eps=1e-05, affine=True)
        self.emb4 = nn.Linear(4, N*2)
        self.convb1_1 = nn.Conv2d(N*2, N*4, kernel_size=3, padding=0, bias=False)
        self.normb1_2 = nn.GroupNorm(N*2, N*4, eps=1e-05, affine=True)
        #relu
        self.convb1_2 = nn.Conv2d(N*4, N*4, kernel_size=3, padding=0, bias=False)
        self.normb1_3 = nn.GroupNorm(N*2, N*4, eps=1e-05, affine=True)
        #relu

        '''Up Block 1'''
        self.up1 = nn.ConvTranspose2d(N*4, N*2, kernel_size=2, stride=2) # Deconvolution method for upsampling# Use for deconvolution method of upsampling
        self.normu1_1 = nn.GroupNorm(N, N*2, eps=1e-05, affine=True)
        self.emb5 = nn.Linear(4, N*4)
        self.convu1_1 = nn.Conv2d(N*4, N*2, kernel_size=3, padding=0, bias=False)
        self.normu1_2 = nn.GroupNorm(N, N*2, eps=1e-05, affine=True)
        #relu
        self.convu1_2 = nn.Conv2d(N*2, N*2, kernel_size=3, padding=0, bias=False)
        self.normu1_3 = nn.GroupNorm(N, N*2, eps=1e-05, affine=True)
        #relu
