...
class UNetLite_hls(tf.keras.Model):
    def __init__(self, c_in=1, time_dim=4):
        ...
        N = 128 # Change number of kernels at each layer here
    ...
    def call(self, inputs):
        ...
        # Down 2
        xd2 = CustomPadding(1, 1)(xd1)
        xd2 = self.pool1(xd2)
        xd2 = self.normd2_1(xd2)
        xd2 = self.relu(xd2)
        emb2 = tf.tile(self.emb2(t), [1, tf.shape(xd2)[1], tf.shape(xd2)[2], 1])
        xd2 = CustomPadding(1, 1)(emb2 + xd2)
        xd2 = self.convd2_1(xd2)
        xd2 = self.normd2_2(xd2)
        xd2 = self.relu(xd2)
        xd2 = CustomPadding(1, 1)(xd2)
        xd2 = self.convd2_2(xd2)
        xd2 = self.normd2_3(xd2)
        xd2 = self.relu(xd2)

        # Down 3
        xd3 = CustomPadding(1, 1)(xd2)
        xd3 = self.pool2(xd3)
        xd3 = self.normd3_1(xd3)
        xd3 = self.relu(xd3)
        emb3 = tf.tile(self.emb3(t), [1, tf.shape(xd3)[1], tf.shape(xd3)[2], 1])
        xd3 = CustomPadding(1, 1)(emb3 + xd3)
        xd3 = self.convd3_1(xd3)
        xd3 = self.normd3_2(xd3)
        xd3 = self.relu(xd3)
        xd3 = CustomPadding(1, 1)(xd3)
        xd3 = self.convd3_2(xd3)
        xd3 = self.normd3_3(xd3)
        xd3 = self.relu(xd3)

        # Bottleneck 1
        xb1 = CustomPadding(1, 1)(xd3)
        xb1 = self.pool3(xb1)
        ...
        xu1 = CustomPadding(1, 1)(emb5 + Concatenate(axis=-1)([xu1, xd3]))  # Concatenate along the last axis
        ...
        xu2 = self.relu(xu2)
        emb6 = tf.tile(self.emb6(t), [1, tf.shape(xu2)[1], tf.shape(xu2)[2], 1])
        xu2 = CustomPadding(1, 1)(emb6 + Concatenate()([xu2, xd2]))
        xu2 = self.convu2_1(xu2)
        xu2 = self.normu2_2(xu2)
        xu2 = self.relu(xu2)
        xu2 = CustomPadding(1, 1)(xu2)
        ...
        xu3 = self.relu(xu3)
        emb7 = tf.tile(self.emb7(t), [1, tf.shape(xu3)[1], tf.shape(xu3)[2], 1])
        xu3 = CustomPadding(1, 1)(emb7 + Concatenate()([xu3, xd1]))
        xu3 = self.convu3_1(xu3)
        xu3 = self.normu3_2(xu3)
        xu3 = self.relu(xu3)
        xu3 = CustomPadding(1, 1)(xu3)
        xu3 = self.convu3_2(xu3)
        xu3 = self.normu3_3(xu3)
        xu3 = self.relu(xu3)
